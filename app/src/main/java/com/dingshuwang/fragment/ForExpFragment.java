 ...
 */
public class ForExpFragment extends BaseFragment implements DataView {

    @Bind(R.id.listview)
    ListView listView;

    public static ForExpFragment newInstance(){
        return new ForExpFragment();
    }
    ...
    private void doData(){
        String url = String.format(APIURL.FOR_EXP, Constants.USER_ID);
    ...
    public void onGetDataSuccess(String result, String requestTag) {
        if(null != result){
            ...
            if(null != item && null != item.shops){
                listView.setAdapter(new TopayAdapter(item.shops,mActivity,3));
            }
        }
    }
}
