...
public class AllOrderFragment extends BaseFragment implements DataView{
    ...
    private void doData(){
        String url = String.format(APIURL.ALL_ORDER_URL, Constants.USER_ID);
    ...
    public void onGetDataSuccess(String result, String requestTag) {
        if(null != result){
            ...
            if(null != item && null != item.shops){
                listView.setAdapter(new TopayAdapter(item.shops,mActivity,1));
            }
        }
    }
